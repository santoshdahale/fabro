...
The `@` prefix tells Arc to load the prompt from a file path relative to the workflow file. Paths support `~` (home directory) and `..` (parent directory):
...
Arc validates `@file` references at parse time — if the referenced file does not exist, validation fails with a clear error pointing to the bad reference.
